...
- Context Lineage can now capture a real supported-provider request family through a temporary local relay, retain its full encoded bytes only in a mode-`0600` uncommitted fixture, and verify exact base-to-leaf extension with digest-only output.
- Context Lineage's live Gate B probe now supports independent repeated samples, emits redacted aggregate reviewer deltas, and reports the durable session containing each raw grounded-plan sidecar artifact.
- Context Lineage bounded phrase retrieval now ranks a capped set of executable source candidates by distinct task-term coverage in a fixed local window, keeps direct task-path owners ahead of supporting surfaces, and preserves the verification classification of tests whose own contents match the task.
- Parallel Questions board entry and its “Ask from here” editor now require an idle, stable session leaf, preventing a streamed or compacting parent turn from being forked implicitly.
- Repository planning now applies the configured bounded temporal and read-only adapter profile; `--current-state-only` continues to explicitly disable both.
- Repository planning accepts compact, deterministic evidence aliases from its rendered catalog and normalizes them only to the frozen manifest's immutable IDs, reducing citation-copy failures without expanding planning authority.
- Caller-authorized forge artifacts can now enter Context Lineage’s bounded documentary boundary with explicit documentary authority, unknown staleness, and no live network retrieval or current-state elevation.
- Repository planning now renders a compact source-to-evidence-ID catalog beside the immutable manifest, improving exact citation affordance without weakening frozen-manifest validation.
- Parallel Questions retry now retains completed siblings and dispatches only missing task identities, including after a completed-but-partial cancellation run.
- The Parallel Questions board copy action now copies completed sidecar answer bytes rather than an `artifact://` reference, without inserting raw answers into parent history.
- Active Parallel Questions runs now remain available to the interactive board while sidecar tasks execute; `x` cancels the selected task through its isolated abort handle without changing parent context or stopping siblings.
- Context Lineage's shared coordinator now accepts item-specific abort signals, so cancellation can reject one fan-out item without aborting independent siblings.
- The Context Lineage Parallel Questions board now navigates every durable run instead of showing actions only for the newest run; actions remain scoped to the selected persisted run.
- Context Lineage bounded current-source retrieval now uses a capped native exact-task-phrase seed across bounded source roots to surface generic-named source owners without widening its lexical scan budget; exhaustion and native-search failure remain explicit degraded-source records.
- Context Lineage repository planning makes one bounded static-prompt repair attempt after malformed JSON responses, while semantic/evidence validation failures remain non-retriable.
- Failed Context Lineage benchmark planning responses now persist as session sidecar artifacts and the Gate B harness reports their references, so invalid provider output is reviewable instead of being lost after a console-only failure.
- Context Lineage bounded retrieval now keeps the invoking package workspace in scope, prioritizes current source over examples/docs/tests, normalizes common task inflections, and never lets relative-import expansion overwrite a task-primary candidate. This recovers the relevant `AgentSession` fork source for the CL-02 planning benchmark while preserving explicit cross-workspace paths and dependency expansion.
- Fixed the Context Lineage Gate F live-provider harness fixture path so it reads package source correctly before running cache observations.
- Current-state retrieval now always evaluates bounded semantic content matches alongside path matches; a loose filename hit can no longer suppress the implementation source that actually matches the task.
- `/lineage ask` plans now root at the derived manifest checkpoint instead of the raw manifest; FR51 evidence-or-assumption validation rejected every lowered ask plan rooted at a `repository_manifest` base.
- Plan execution no longer throws "cannot make dependency progress" when failure blocking removes the last pending stage downstream of a failed stage; the run terminates with failed status and completed outputs preserved.
- Lineage records force session-file durability on append: session files are created lazily on the first assistant message, so compile-only lineage flows that crashed before any model turn silently lost their manifest/checkpoint/progress records. Stage-progress markers are awaited durable before the next stage starts.

### Added

- Context Lineage now documents and exposes its safe retention policy: immutable named-base versions are logically collectable only after all active-name, plan, and execution references clear; physical cleanup remains the existing session-scoped delete operation so shared artifact IDs are never pruned unsafely.
- Gate E now has a redaction-safe local fixture verifier for authorized provider captures, reporting only prefix digests and exact base-to-leaf extension results rather than provider-visible request bytes.
- `/lineage benchmark all` now executes and persists the declared representative corpus case-by-case, reporting every valid/rejected case and mean scope, verification, and traceability deltas without changing the default-off gate.
- `/lineage base retention` now exposes named-base checkpoint refcounts and logical collectability, while identifying session deletion as the safe physical cleanup boundary.
- `/lineage documentary <local|forge> <authorized-file> :: <task>` creates a bounded, documentary-authority plan from an explicit local or already-authorized forge artifact; it never performs network retrieval or promotes documentary material to current repository truth.
- Context Lineage retrieval now scans a separately bounded set of task-relevant test files and reserves only those whose own contents match the task, making verification evidence available without replacing the source-first retrieval budget.
- Parallel Questions result management: fan-out leaves now persist named outputs so `/lineage synthesize <run-id>` can reduce completed sidecar artifacts without rerunning questions; `/lineage diagnostics <run-id>` reports durable run/cache metadata, and the interactive board exposes synthesis and diagnostics alongside inspect, copy, retry, promote, and discard.
- `/fanout <q1> | <q2> [| …]` as the canonical Parallel Questions command (with `/lineage ask` retained), named-base inspection/deletion tombstones plus checkpoint retention accounting, bounded Git co-change evidence labeled as statistical, and local caller-authorized documentary evidence labeled non-current and documentary.
- Context Lineage controlled-reasoning and scheduling substrate: declared candidate variations, blinded evaluation views, durable selection provenance, evaluated budget-bound continuation decisions, semantic-priority/deadline-first locality tie-breaking, and observe-only conditional-preparation records.
- External evidence adapters (PR 3A): read-only Graphify `graph.json` and SCIP/local-index adapters normalize existing artifacts into bounded, candidate-authority evidence — `EXTRACTED` edges stay candidate current-structural claims while `INFERRED`/`AMBIGUOUS` edges can only ever be statistical inspection scope. Artifacts whose source boundary mismatches the frozen snapshot are rejected under `contextLineage.repositoryContext.adapters.requireSnapshotMatch` (default on) or included with explicit stale labels; unknown schema versions fail at the boundary; malformed artifacts degrade into failed-collection records without blocking planning; and record order never changes normalized identity. `auto` mode (the SCIP default) consumes an already-present `.scip/index.scip` or `index.scip` and never installs, generates, or contacts a service.
- Planning-skill boundary hardening: evidence references citing a repository path are canonicalized onto the matching manifest item's semantic ID before validation, and the skill prompt now forbids path-as-ID citations, so real-model plans stop failing on identifier style while unknown references still fail validation.
- Gate verification harnesses under `scripts/context-lineage-gates/`: real-provider fan-out cache measurement (Gate F), NFR4 SIGKILL/resume probe, PR 3A adapter ablation, Gate B reviewer packet, and promotion idempotency against a persisted SessionManager.
- Adapter orchestration: `/lineage <task>` runs enabled adapters before temporal enrichment, reports per-adapter outcomes (`included`, observed-only, rejected, unavailable, failed) in command output, and merges adapter evidence under its own budget cap with an explicit degraded record when truncated. Observed-only candidates never change the manifest identity.
- `compareAdapterEvidenceAblations` compares native-only versus adapter-enriched manifests against the same benchmark obligations, separating unique useful coverage from duplicates, stale facts, and unsupported-scope growth so adoption decisions cannot be argued from evidence volume alone.
- Milestone telemetry (§20.1): shared `lineageEvent` boundary plus new `repository_evidence_selected`, `prepared_prefix_created`, `plan_validated`, and adapter lifecycle events (`evidence_adapter_started/completed/degraded/rejected`) with opaque identifiers and counts only.

- `/wayfinder plan <map-url> <ticket-url> :: <goal> :: <task>` resolves read-only GitHub Wayfinder map and ticket snapshots through the local cache, binds them to a Campaign goal and frozen repository manifest/checkpoint, and persists a manifest-validated ticket plan without tracker mutations.
- `/lineage <task>` compiles and records a frozen, read-only repository context and logical checkpoint; `/lineage show [manifest-id]` safely inspects selected evidence and omissions; `/lineage plan <task>` generates a manifest-validated plan; `/lineage benchmark <case-id>` records a grounded-versus-task-only comparison; and `/lineage status` reports durable lineage counts without rendering source evidence. These surfaces live on a dedicated command (the `/context` diagnostic keeps its original meaning) and are gated behind `contextLineage.enabled` (default off) until the plan-quality gate passes.
- Context Lineage persists digest-only manifest, checkpoint, validated-plan, and content-addressed execution records in session history; source excerpt bytes stay in the session artifact store referenced by artifact ID, so forked transcripts never duplicate repository source.
- Repository snapshot overlay hashing obeys a dirty-file budget (`maxDirtyFiles` in the retrieval policy); exceeding it records an explicit degraded source and discloses the truncation wherever the snapshot identity is shown.
- Repository planning benchmarks persist reviewer adjudication — reviewer identity, mode, rules version, and post-normalization scores — beside the pre-normalization automated scores; no enablement decision cites unreviewed automated scores alone.
- `lowerFanoutRequest` deterministically lowers a compact parallel-questions request into a one-stage lineage plan: identical requests produce identical plan identities, question order is preserved, no provider-specific fields are introduced, and request validation shares its rules with the plan validator.
- `createCheckpointExtensionCheckpoint` creates deterministic next-generation checkpoints from selected upstream outputs (hash covers base checkpoint plus canonically serialized output digests), with `serializeCheckpointExtensionOutputs` providing the canonical text downstream runners embed.
- Plan execution now persists a per-stage progress record (grouped by `runId`) after each stage completes, and `executeContextLineagePlan` accepts a `resume` input of recovered stage completions so a crashed or failed run skips finished stages instead of rerunning them.
- `partitionCompatibilityFamilies` groups plan stages into explained prefix families for one execution target: stages sharing a base and capability signature join one family, differing requirements form separate families, and requirements unavailable to the caller become explicit divergences with reasons.
- Opt-in bounded temporal enrichment (PR 9A slice): `collectTemporalEvidence` derives rename-lineage (`historical_observation`) and churn (`statistical_relationship`) evidence from a single budgeted Git history scan via the new `git.log.nameStatus` helper; `mergeTemporalEvidence` rebuilds the manifest with a distinct merged retrieval policy, and `/lineage` enriches when `contextLineage.repositoryContext.temporal` is enabled. Missing or shallow history degrades explicitly instead of failing planning.
- Named checkpoint bases (PR 9 slice): `createNamedBaseRecord` names a checkpoint idempotently per (name, checkpoint), `resolveNamedBase` returns the latest unarchived generation, `archiveNamedBaseRecord` tombstones without deleting history, and `/lineage status` reports resolvable named-base counts. `/lineage base <name>` names the latest checkpoint, `/lineage base list` shows active bases, and `/lineage base archive <name>` retires one. Derived checkpoint generations (extensions) now recover from session history scoped to their root manifest.
- Context utility signals (FR38 foundation): `contextUtilityByCheckpoint` separates checkpoint reuse (execution counts) from usefulness signals (completed runs, persisted plans) so reuse counts are never treated as value.
- `named_base` plan sources (PR 9): plans can root their bases at a named checkpoint; `resolvePlanNamedBases` rewrites names to concrete checkpoints from the latest active session records before validation, and unresolvable or archived names fail validation instead of silently substituting a base. `executeContextLineagePlan` performs this resolution itself, so plans rooted at named bases execute end-to-end.
- `/lineage answers [run-id]` inspects completed ask runs: per-answer question text (from the persisted plan), task id, output name, first-line preview from the sidecar artifact, artifact reference, and observed cache status when recorded. The question cap reads `contextLineage.fanout.maxItems` (default 5).
- `/lineage plan --run --execute <plan-id>` executes an already-persisted plan fresh through the side-request runner after verifying its manifest integrity, listing resulting answers with artifact references.
- `/lineage plan --run --resume <run-id>` recovers a failed plan run's persisted manifest/checkpoint/plan and completed stages from session records, verifies manifest integrity, and reruns only unfinished stages (FR24).
- `/lineage ask <q1> | <q2> [| …]` runs Parallel Questions headless (PR 4): lowers 2–5 independent questions into a one-stage plan rooted at a fresh frozen manifest and executes them as staggered no-tools side requests; answers stay in sidecar artifacts in input order and the parent context is unchanged.
- Side-request plan execution (PR 4/5 headless slice): `createSideRequestContextLineageTaskRunner` dispatches tasks as no-tools ephemeral turns, persists raw answers to the artifact store, and returns digest + `artifact://` references verifiable against session artifacts; `/lineage plan <task> --run` executes the persisted plan through it with the parent context untouched.
- `lineage.result_promoted` and `lineage.synthesis_created` events complete the promotion and synthesis lifecycle telemetry.
- `lineage.repository_manifest_degraded` and `lineage.evidence_adapter_started/completed/degraded` events complete the §20.1 Milestone A–B telemetry family, covering degraded manifests and adapter lifecycle.
- `lineage.item_attempt_started/completed/failed` events per task with run/plan/stage/task identifiers and failure details, completing the §20.1 attempt-level event family.
- Milestone telemetry (§20.1–§20.2): `lineage.*` debug events (snapshot frozen, manifest created, checkpoint created, run started, stage ready/completed, plan compiled) emit opaque identifiers and §20.2 dimensions — evidence-class counts, degraded/omission counts, warm/failure policies, fan-out width — with no raw prompts, answers, or unscoped hashes.
- §25 sub-key gates: `contextLineage.fanout.enabled`, `contextLineage.plans.enabled`, and `contextLineage.repositoryContext.enabled` (all default on, layered under the master `contextLineage.enabled` gate) independently control the ask, plan/benchmark, and repository-context surfaces.
- Fanout requests enforce the FR10 first-release cap of five questions in `validateFanoutRequest` (configurable per call, `contextLineage.fanout.maxItems`), and `/lineage benchmark list` enumerates available benchmark cases. Side-request concurrency follows `contextLineage.fanout.maxConcurrency` (FR13).
- `firstPrefixDivergence` reports the first provider-visible difference between two compiled request views (kind change, content divergence, or one-sided extension) as the diagnostic primitive for exact-prefix family checks (FR3).
- Synthesis stages (PR 6): plans can declare `mode: "synthesis"` reducer stages with result selectors (`successful`/explicit task ids) that consume upstream outputs and produce one named output for downstream extension bases; validation requires declared dependencies and unique output names, and the executor runs the reducer through the same runner/verifier/progress-record path as leaf stages.
- Plan failure policies (§11.8/§21.5): `defaults.failurePolicy` (`continue_independent`, `stop_dependents` — the default, and `stop_plan`) plus a per-run override on `executeContextLineagePlan`. `continue_independent` keeps successful sibling outputs and lets dependents try with what survived; `stop_dependents` blocks only stages downstream of a failure while unrelated stages still run; failed and blocked stages fail the run without discarding completed work.
- `/lineage promote <run-id> <answer-number>` promotes a completed run's answer through the PR 7 contract: the ask flow records its origin session leaf on the execution record, and promotion appends the assignment plus sanitized answer under that leaf via `appendMessageToBranch` — never touching the active leaf — with idempotent, digest-verified provenance.
- Origin-safe result promotion (PR 7): `promoteContextLineageResult` appends the assignment plus sanitized answer to a caller-resolved origin branch — never the merely-active leaf — records digest-only provenance (origin leaf, assignment/answer digests, artifact reference, resulting session and leaf) in session history, and is idempotent per (origin, assignment, answer): repeated requests return the recorded branch instead of duplicating it.
- Measured-cost economics helpers (§20.4–§20.5): `fanoutSharedCost` compares uncached fan-out shared-prefix cost with observed cached cost (writers × write multiplier + readers × read multiplier), and `planInducedReuse` compares compiled plan execution against the per-task serialization counterfactual; ambiguous accounting stays the caller's exclusion, never a silent claim. `/lineage status` now also reports promotion counts.
- Provider adapter boundary (PR 4 logic layer, NFR5/NFR6): `LineageProviderAdapter` keeps all cache-relevant encoding behind one contract; `FakePrefixCache` + `FAKE_LINEAGE_PROVIDER` model exact prefixes, duplicate writers, hits/misses, and TTL expiry deterministically; `verifyExactFamily` proves every leaf request extends the prepared base and reports the first provider-visible divergence (kind change, content rewrite, or truncated leaf) instead of falsely classifying it as exact.
- Cache observation plumbing (FR8): task results may carry a provider-reported `cacheObservation`; the executor surfaces observations on the outcome, persists per-output cache status in execution records, and emits `lineage.prefix_observed` telemetry with read/write token counts — expected compatibility, provider-observed reuse, and unknown behavior stay distinct facts.
- Warm coordination (§18.3): `executeContextLineagePlan` accepts `warmPolicy: "stagger_first"` — the first task of each stage dispatches alone so its cache write lands before siblings read. `/lineage ask` reads `contextLineage.fanout.warmPolicy` (default `off`) per the PRD's measure-before-enabling rule. Cold-start duplicate-writer behavior remains measurable through `FakePrefixCache`.
...
- Fixed `/live` dropping structured Codex error details before writing its debug log, so server-provided error codes and metadata are now available in `~/.omp/logs/`.
