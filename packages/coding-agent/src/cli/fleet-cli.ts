...
function targetName(t: MuxSeat["target"]): string {
...
async function fetchMuxStatus(gatewayUrl?: string): Promise<MuxStatusResponse | null> {
	...
	try {
		...
		return typeof status.stickySessions === "number" && typeof status.policy?.weeklyCloseAt === "number"
			? status
			: null;
	} catch {
		return null;
...
async function syncSessionMuxDecisions(): Promise<void> {
	...
	try {
		await Bun.write(SCANNED_CACHE_PATH, JSON.stringify([...fullyScanned]));
	} catch {}
}
...
export async function runFleetStatus(args: {
	limit?: number;
	gatewayUrl?: string;
	routingLog?: string;
}): Promise<void> {
	...
	try {
		const [decisions, mux] = await Promise.all([
			fetchRecentDecisions(limit, args.routingLog),
			fetchMuxStatus(args.gatewayUrl),
		]);
		...
		const tokens =
			pTotal > 0 ? ` · ${formatTokens(pTotal)} in +${formatTokens(cTotal)} cache + ${formatTokens(oTotal)} out` : "";

		console.log(
			`Fleet routing · ${decisions.length} recent · mux cheap ${cheap} / capable ${capable}${tierLabel}${tokens}`,
		);
		...
		if (mux) {
			const seats = [
				...
				.map(s => `${s.open ? "open" : (s.reason ?? "closed")} ${targetName(s.target)}`)
				.join(" · ");
			console.log(
				`Mux · ${mux.stickySessions} sticky · close 5h ${(mux.policy.fiveHourCloseAt * 100).toFixed(0)}% / week ${(mux.policy.weeklyCloseAt * 100).toFixed(0)}%${seats ? ` · ${seats}` : ""}`,
			);
		} else {
